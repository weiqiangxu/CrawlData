	...
	public static function move()
	{
		...
		while(!$empty) {
	        ...
	        foreach ($datas as $data) {
	        	...
	        	// 唯一校验
	        	$empty = Capsule::connection('yp_realoem')->table('data_product')->where('pro_realoem',$data->part_detail_num)->get()->isEmpty();
	        	if(!$empty) continue;
	        	...
	        	// 2、vin表
	        	$data_vin = Capsule::connection('yp_realoem')->table('data_vins')->where([['vin_name',$vin],['vin_etk_id',$data_etks['etk_id']]])->orderBy('vin_id','desc')->first();
			    if(empty($data_vin)){
			    	// 获取主键
			    	$res = Capsule::connection('yp_realoem')->table('data_vins')->orderBy('vin_id','desc')->first();
			    	if(!empty($res)){$vin_id=($res->vin_id<8269)?8269:$res->vin_id+1;}else{$vin_id=8269;}
			    	$temp = ['vin_id'=>$vin_id,'vin_etk_id'=>$data_etks['etk_id'],'vin_name'=>$vin,'vin_read'=>1];
			    ...
			    $temp = array(
					...
					'pro_post' => $pro_pos,// data_etks 表 etk_pinyin字段 + 号码名称拼音（overtrue/pinyin）小写下划线 + 号码的去格式小写
					'pro_seo' => 1,
					'pro_yp_name' => ''
			    );
			    ...
			    $temp = array(
					...
					'ppm_pmg_id' => $pmg_id,// 组级
					'ppm_ppg_id' => 0,// 图片
					'ppm_pos_id' =>'',// 位置
					'ppm_txt_id' => $data_text_id,// 描述
					'ppm_order' => 999
			    );
			    Capsule::connection('yp_realoem')->table('data_product_partgrp_8')->insert($temp);
			    // 标记已读
				Capsule::table('part_detail')->where('id', $data->id)->delete();
